    ...
    struct char_tab_head *resource_list, int x, int y)
{
    ...
    for (int i = 0; need_to_generate_more_resource(i, x, y, FOOD_DENSITY); i++) {
    ...
    struct char_tab_head *resource_list, int x, int y)
{
    ...
    for (int i = 0; need_to_generate_more_resource(i, x, y, LINEMATE_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
        resource->str = strdup("linemate");
        TAILQ_INSERT_TAIL(resource_list, resource, next);
    }
    for (int i = 0; need_to_generate_more_resource(i, x, y, DERAUMERE_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
        resource->str = strdup("deraumere");
        TAILQ_INSERT_TAIL(resource_list, resource, next);
    }
    for (int i = 0; need_to_generate_more_resource(i, x, y, SIBUR_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
    ...
    struct char_tab_head *resource_list, int x, int y)
{
    ...
    for (int i = 0; need_to_generate_more_resource(i, x, y, MENDIANE_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
        resource->str = strdup("mendiane");
        TAILQ_INSERT_TAIL(resource_list, resource, next);
    }
    for (int i = 0; need_to_generate_more_resource(i, x, y, PHIRAS_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
        resource->str = strdup("phiras");
        TAILQ_INSERT_TAIL(resource_list, resource, next);
    }
    for (int i = 0; need_to_generate_more_resource(i, x, y, THYSTAME_DENSITY); i++) {
        resource = calloc(sizeof(char_tab_t), 1);
