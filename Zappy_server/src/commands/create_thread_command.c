                            ...
                            char *user_uuid)
{
    ...
    dprintf(client_fd, "200|/create%sthread%s%s%s%s%s%s%s%s%s%s%s%s", END_LINE,
            END_LINE, new_thread->thread_uuid, SPLIT_LINE, user_uuid,
            SPLIT_LINE, timestamp, SPLIT_LINE, new_thread->title, SPLIT_LINE,
            new_thread->body, END_LINE, END_STR);
    return OK;
}
                        ...
                        all_context_t *all_context)
{
    ...
    write_new_thread(
        teams_server->actual_sockfd, new_thread,
        teams_server->clients[teams_server->actual_sockfd].user->uuid);
    return OK;
}
